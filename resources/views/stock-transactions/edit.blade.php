@php
    ...
    $defaultLines = old('lines', $transaction->lines->map(fn ($l) => [
...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Edit {{ $transaction->document_number }}</h2>
    </x-slot>
    <div class="py-10">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('stock-transactions.update', $transaction) }}"
                  x-data="{
                ...
                <div>
                    ...
                    <x-text-input id="occurred_on" name="occurred_on" type="date" class="mt-1 block w-full" :value="old('occurred_on', $transaction->occurred_on->format('Y-m-d'))" required />
                </div>

                <div>
                    <x-input-label for="notes" value="Catatan" />
                    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 text-sm">{{ old('notes', $transaction->notes) }}</textarea>
                </div>

                @if ($type === StockTransactionType::Adjustment)
                    <p class="text-xs text-amber-800 bg-amber-50 ring-1 ring-amber-100 rounded-lg p-3">
                        Penyesuaian: gunakan jumlah positif / negatif sesuai kebutuhan.
                    </p>
                @endif
                    ...
                    <template x-for="(line, index) in lines" :key="index">
                        <div class="grid gap-3 sm:grid-cols-12 items-end border border-gray-100 rounded-lg p-3">
                            ...
                            <div class="sm:col-span-3" x-show="showUnitCost" x-cloak>
                                <label class="text-xs text-gray-500">Harga / unit</label>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('stock-transactions.show', $transaction) }}" class="self-center text-gray-600 hover:underline">Batal</a>
                </div>
            </form>
        </div>
    </div>
