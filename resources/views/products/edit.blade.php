<x-app-layout>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-4 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100 text-sm">
                @csrf @method('PUT')
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="unit_id" name="unit_id" class="mt-1 block w-full rounded-md border-gray-300 text-sm" required>
                            @foreach ($units as $u)
                                <option value="{{ $u->id }}" @selected(old('unit_id', $product->unit_id) == $u->id)>{{ $u->name }} ({{ $u->code }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="sm:col-span-2">
                        <x-input-label for="description" value="Deskripsi" />
                        <textarea id="description" name="description" rows="2" class="mt-1 block w-full rounded-md border-gray-300 text-sm">{{ old('description', $product->description) }}</textarea>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <x-text-input id="sell_price" name="sell_price" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('sell_price', $product->sell_price)" required />
                    </div>
                    <div>
                        <x-input-label for="min_stock_level" value="Minimum stok" />
                        <x-text-input id="min_stock_level" name="min_stock_level" type="number" step="0.0001" min="0" class="mt-1 block w-full" :value="old('min_stock_level', $product->min_stock_level)" required />
                    </div>
                </div>
                <label class="inline-flex items-center gap-2 text-gray-700">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $product->is_active)) class="rounded border-gray-300 text-indigo-600"> Aktif
                </label>
                <div class="flex gap-3">
                    <x-primary-button type="submit">Perbarui</x-primary-button>
                    <a href="{{ route('products.show', $product) }}" class="self-center text-gray-600 hover:underline">Batal</a>
                </div>
            </form>
        </div>
    </div>
